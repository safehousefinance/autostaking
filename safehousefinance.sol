...
// SAFEHOUSE FINANCE COPYRIGHT (C) 2022
...
contract SafehouseFinance is IERC20Metadata, Ownable {
    ...
    uint256 public constant SAFIInsuranceFundFee = 50;
    uint256 public constant sellFee = 20;
    uint256 public constant firePitFee = 25;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(SAFIInsuranceFundFee).add(
            firePitFee
        );
    ...
    address public SAFIInsuranceFundReceiver;
    ...
    constructor() IERC20Metadata("SafehouseFinance", "SAFI", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0xCeCe87330892DFf7abB416b4E3978CaA884db3cF;
        treasuryReceiver = 0x087987071df71fFEdbF65590F802E60d16Bbb392;
        SAFIInsuranceFundReceiver = 0xc8a10b06c3cfDe0649462cfb79dDD340a8E7B2b9;
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.mul(_treasuryFee.add(SAFIInsuranceFundFee)).div(
                feeDenominator
            )
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(SAFIInsuranceFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(SAFIInsuranceFundReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(SAFIInsuranceFundFee).div(
                treasuryFee.add(SAFIInsuranceFundFee)
            ),
            gas: 30000
        }("");
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        require(
            amountToSwap > 0,
            "There is no SAFI token deposited in token contract"
        );
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
    ...
    function setFeeReceivers(
        ...
        address _SAFIInsuranceFundReceiver,
        address _firePit
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        SAFIInsuranceFundReceiver = _SAFIInsuranceFundReceiver;
        firePit = _firePit;
    }
